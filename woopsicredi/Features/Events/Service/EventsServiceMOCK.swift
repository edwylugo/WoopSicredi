...
class EventsServiceMOCK : EventsListVMDataProvider, EventDetailVMDataProvider, CheckInVMDataProvider {
    ...
    func fetchEvents(completion: @escaping (WoopRequestErrorModel?, FetchEventsResponse?) -> Void) {
        DispatchQueue.main.asyncAfter(deadline: .now() + 2) { [weak self] in
            guard let weakSelf = self else { return }
            completion(weakSelf.fetchEventsError(),nil)
        }
    }
    ...
    func fetchEventDetails(eventID: String, completion: @escaping (WoopRequestErrorModel?, FetchEventDetailsResponse?) -> Void) {
        DispatchQueue.main.asyncAfter(deadline: .now() + 2) { [weak self] in
            guard let weakSelf = self else { return }
            completion(weakSelf.fetchEventDetailsError(),nil )
        }
    }
    
    //MARK: Fetch Events Scenarios
    func fetchEventsSuccess() -> FetchEventsResponse {
        let response = JSON([[
            "id": "1",
        ...
        return FetchEventsResponse(json: response)
    }
    
    func fetchEventsError() -> WoopRequestErrorModel {
        let error = WoopRequestErrorModel(response: nil)
        error.statusCode = 404
        error.errorDescription = "Fetch events error"
        return error
    }
    
    //MARK: Fetch Event Details Scenarios
    func fetchEventDetailsSuccess() -> FetchEventDetailsResponse {
        let response = JSON([
            "id": "1",
            "title": "Feira de adoção de animais na Redenção",
            "price": 29.99,
        ...
        return FetchEventDetailsResponse(json: response)
    }
    
    func fetchEventDetailsError() -> WoopRequestErrorModel {
        let error = WoopRequestErrorModel(response: nil)
        error.statusCode = 404
        error.errorDescription = "Fetch event detail error"
        return error
    }
    
    ////MARK: Send Check In Scenarios
    
    func sendCheckInError() -> WoopRequestErrorModel {
        let error = WoopRequestErrorModel(response: nil)
        error.statusCode = 405
        error.errorDescription = "Check in error"
        return error
    }
}
